...
export const Header = ({ scrollY }: HeaderProps) => {
  return (
    <header 
      className="fixed top-0 z-50 w-full border-b border-gray-800/30 bg-gray-900/95 backdrop-blur-sm transition-all duration-300"
      style={{
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between py-4 sm:py-6">
          ...
          <nav className="flex space-x-4 sm:space-x-8">
            <Link href="/" className="text-sm font-inter font-medium text-white transition-colors hover:text-gray-300 sm:text-base">作品</Link>
            <Link href="/about" className="text-sm font-inter text-gray-300 transition-colors hover:text-white sm:text-base">プロフィール</Link>
            <Link href="/contact" className="text-sm font-inter text-gray-300 transition-colors hover:text-white sm:text-base">お問い合わせ</Link>
          </nav>
        </div>
      </div>
    </header>
